...
```{r}
gap1 <- gap %>% filter(continent == "Asia" | continent == "Europe")
head(gap1)
str(gap1)
```
...
```{r}
gap2 <- gap %>% mutate(gdp = gdpPercap * pop,
                       le_to_gdp = lifeExp/gdp)
head(gap2)
```
...
```{r}
gap4 <- gap %>% filter(continent == "Americas") %>%
  arrange(desc(pop))
head(gap4)
```
...
```{r}
gap5 <- gap %>% filter(continent == "Asia" & year < 1975) %>%
  arrange(pop)
head(gap5)
```
...
```{r}
gap6 <- gap %>% filter(year == 2007 & lifeExp > 80)
gap6
nrow(gap6)
```
