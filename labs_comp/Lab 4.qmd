...
* What happens if we don't make them factor variables?
  * It treats Sex as a continous variable

```{r}
ggplot(data = ICU, aes(x = Age, y = SysBP, color = Sex)) +
  geom_point()
```
...
* Now sex is a factor variable and it's treated as two categories
...
ggplot(data = ICU, 
  ...
  geom_vline(xintercept = 120, color = "darkgreen") +
  geom_vline(xintercept = 130, color = "yellow", linetype = "dashed") +
  geom_vline(xintercept = 140, color = "red", linewidth = 2)
```
...
* A similar plot uses `geom_jitter()` to display the data
  * Note that this isn't a dot plot and doesn't display the **distributions** in the same way, but it does kind of convey the same information
