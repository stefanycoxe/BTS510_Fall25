...
```{r}
bern_means50 <- data.frame(bern_pop) %>%
  rep_sample_n(size = 50, reps = 10000, replace = TRUE) %>%
  summarise(x_bar = mean(bern_pop))
...
```{r}
bern_means100 <- data.frame(bern_pop) %>%
  rep_sample_n(size = 100, reps = 10000, replace = TRUE) %>%
  summarise(x_bar = mean(bern_pop))
...
```{r}
pois_means50 <- data.frame(pois_pop) %>%
  rep_sample_n(size = 50, reps = 10000, replace = TRUE) %>%
  summarise(x_bar = mean(pois_pop))
...
```{r}
pois_means100 <- data.frame(pois_pop) %>%
  rep_sample_n(size = 100, reps = 10000, replace = TRUE) %>%
  summarise(x_bar = mean(pois_pop))
...
* The distribution gets narrower as sample size increases
...
* The sampling distribution of the *mean* is roughly **normal** regardless of the population distribution
* The sampling distribution becomes more normal as the sample size increases
* The sampling distribution becomes normal a bit more quickly for distributions that are more normal
  * Poisson (many values, kind of bell-shaped, but skewed) becomes normal at smaller sample sizes than the Bernoulli (only two values, very much not normal, not bell-shaped) 
